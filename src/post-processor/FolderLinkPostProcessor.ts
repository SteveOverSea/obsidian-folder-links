...
class FolderLinkPostProcessor {
    ...
    constructor(private folderObs: Observable<IFolderWrapper>) {
    ...
    create(): MarkdownPostProcessor {
        return (element: HTMLElement, context: MarkdownPostProcessorContext) => {
            const folderLinks = element
                ...
                .filter((el) => el.dataset.href && el.dataset.href.endsWith('/'));

            if (folderLinks && folderLinks.length) {
                const child = new FolderLinkView(element, folderLinks);
...
export const folderLinkPostProcessor = (folderObs: Observable<IFolderWrapper>) =>
    new FolderLinkPostProcessor(folderObs).create();
